...
class PgNode_a(PgNode):
    ...
    def precond_s_nodes(self):
        """
        precondition literals as S-nodes (represents possible parents for this
        node).
        It is computationally expensive to call this function; it is only called
        by the class constructor to populate the `prenodes` attribute.
    ...
    def effect_s_nodes(self):
        ...
        It is computationally expensive to call this function; it is only called
        by the class constructor to populate the `effnodes` attribute.
...
class PlanningGraph():
    ...
    def create_graph(self):
        """
        build a Planning Graph as described in Russell-Norvig 3rd Ed 10.3 or 2nd
        Ed 11.4
        ...
        if self.s_levels or self.a_levels:
            raise Exception(
        ...
        for literal in self.fs.pos:
            self.s_levels[0].add(PgNode_s(literal, True))
        for literal in self.fs.neg:
            self.s_levels[0].add(PgNode_s(literal, False))
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            parents = self.s_levels[level] & node_a.prenodes
            if parents == node_a.prenodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (set(node_a1.action.effect_add)
                    & set(node_a2.action.effect_rem)
                or set(node_a1.action.effect_rem)
                    & set(node_a2.action.effect_add))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (set(node_a1.action.effect_add)
                    & set(node_a2.action.precond_neg)
                or set(node_a1.action.effect_rem)
                    & set(node_a2.action.precond_pos)
                or set(node_a2.action.effect_add)
                    & set(node_a1.action.precond_neg)
                or set(node_a2.action.effect_rem)
                    & set(node_a1.action.precond_pos))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for node in node_a2.parents:
            if node.mutex & node_a1.parents:
                return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return node_s1.symbol == node_s2.symbol and node_s1 != node_s2
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        if any({node_s1, node_s2} <= set(node_a.children)
               for node_a in node_s1.parents | node_s2.parents):
            return False
        return all(
                node_a1.is_mutex(node_a2)
